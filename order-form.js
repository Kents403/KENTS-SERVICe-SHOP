// order-form.js - To'liq to'g'irlangan versiya
...
class OrderForm {
    ...
    init() {
        ...
        // DOM tayyor bo'lganda formani render qilish
    ...
    bindEvents() {
        ...
        // Orqaga tugmasi - To'g'rilangan versiya
        document.getElementById('backBtn')?.addEventListener('click', (e) => {
            e.preventDefault();
            this.goBack();
        });
        
        // Keyingi tugmasi - To'g'rilangan versiya
        ...
        if (cartIcon) {
            cartIcon.addEventListener('click', () => {
                setTimeout(() => {
                    this.loadCart();
                    this.renderForm();
                    this.showStep(1); // Har doim 1-qadamdan boshlash
                }, 100);
            });
        }
    ...
    async submitOrder() {
        ...
        try {
            ...
            // 3. Telegramga avtomatik yuborish - ENDI EMAIL GA EMAS, FAQAT TELEGRAM GA
            this.sendOrderToTelegram(orderData);
            
            // 4. OrderSync ga saqlash
            if (window.orderSync) {
                window.orderSync.saveToSync(orderData);
            }
            
            // 5. OrderTracker ga saqlash
            ...
            // 6. Xabarnoma ko'rsatish
            this.showNotification(`✅ Buyurtma #${orderId} Telegramga yuborildi!`);
            
            // 7. 2 soniyadan so'ng savatni tozalash va modalni yopish
            setTimeout(() => {
                // Savatni tozalash
                this.cart = [];
    ...
    sendOrderToTelegram(orderData) {
        try {
            ...
            orderData.items.forEach((item, index) => {
                message += `${index + 1}. ${item.name} - ${item.quantity} dona (${(item.price * item.quantity).toLocaleString()} UZS)\n`;
            });
